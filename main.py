...
def createtasks(name, listn):
    ...
    if listn == 1:
        newttask = tk.Checkbutton(todoframe, text=name)
        newttask.grid(row=todotasks, column=0)
    
    elif listn == 2:
        newptask = tk.Checkbutton(progressframe, text=name)
        newptask.grid(row=progresstasks, column=0)
    
    elif listn == 3:
        newdtask = tk.Checkbutton(doneframe, text=name)
        newdtask.grid(row=donetasks, column=0)
...
todoradio = tk.Radiobutton(creationframe, text="Things to do", variable=horrible, value=1, command=lambda:typetaskfunc(1))
todoradio.grid(row=1, column=0, padx=5, pady=5)
progressradio = tk.Radiobutton(creationframe, text="In progress", variable=horrible, value=2, command=lambda:typetaskfunc(2))
progressradio.grid(row=2, column=0, padx=5, pady=5)
doneradio = tk.Radiobutton(creationframe, text="Already done", variable=horrible, value=3, command=lambda:typetaskfunc(3))
